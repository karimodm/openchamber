...
`messageQueueStore.ts` keeps a queued message until its own send resolves, so between dispatch and resolution the entry is still visible to every reader. Dispatchers must therefore atomically claim the persisted item with `markSending`, read `getSendableQueue()` (or filter `sendingIds`), and release it with `clearSending`; otherwise a composer submit can merge a message the auto-send hook is already delivering and send it twice (the window is seconds over a relay). A composer submit never merges its current text with queued text and drains at most the FIFO queue head, so one OpenCode message ID always names one exact payload and an outcome-unknown head blocks later items. Protected items cannot be edited back into the composer. `clearQueue()` retains in-flight entries for the same reason, while confirmed session deletion uses `purgeQueue()` to remove the target unconditionally. `sendingIds` remains page-local. Queue consumers serialize dispatch across browser documents with a per-target Web Lock. Browsers without Web Locks decline dispatch ownership rather than rely on a timer-backed lease; short state transactions may use a heartbeat-backed local-storage bakery lock, while non-browser tests use a process-local fallback. Every persisted queue mutation rehydrates under one whole-store lock because Zustand stores all targets in one local-storage value; a per-target write based on a stale snapshot would otherwise overwrite changes for another session. Storage events rehydrate already-open documents so a queue written before another tab closes is still discovered. Queue persistence is synchronous once that transaction owns the store. Before either automatic dispatch or an explicit composer dispatch of queued content crosses the network, the queued item durably records its OpenCode message ID in `sendAttempt`; a missing item or deletion tombstone cancels dispatch. Capacity eviction never removes an in-flight or outcome-unknown item. If the page loses the request promise, the next page confirms that exact message through `session.message` and removes the queue entry only after confirmation. A missing message or failed lookup keeps the item queued and blocks automatic re-send because OpenCode does not provide atomic idempotent prompt admission; removing or explicitly retrying the item remains a user decision. A definite rejection clears a newly created `sendAttempt` and follows the normal bounded retry path, while an unresolved ambiguous transport failure or failed retry of an older attempt retains it and switches to exact-ID reconciliation. Confirmed deletion immediately purges the queue and persists a target tombstone without waiting for transport, so late in-flight work or an old composer cannot resurrect the deleted queue.

New message content joins the queue behind an existing head instead of overtaking it. Composer-local slash actions execute under the target lock but are not persisted as messages. Message-producing magic commands persist both their visible text and synthetic instruction. A persisted `sendAttempt` starts as prepared and becomes dispatched only at the transport boundary. A reload retries a prepared identity but only reconciles a dispatched one.

Browser queue admission fails unless raw durable storage confirms the new entry; an in-memory safe-storage fallback must not imply that a message can survive reload. Explicit per-item sends may pass ordinary queued predecessors, but never a predecessor already sending or awaiting exact-ID reconciliation.

Deletion tombstones are retained for 30 days, and version 6 rewrites older persisted queue state through that retention rule.
